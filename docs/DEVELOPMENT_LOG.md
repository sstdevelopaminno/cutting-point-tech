...
## 2026-09-21
- Refined the CpIPOS downloads page by separating the platform card component into `src/components/downloads/DownloadPlatformCard.tsx`, restyling Windows and Android into clearer platform sections with matching icons, adding jump links for `#windows` and `#android`, and removing the visible RC/DEBUG warning copy from the page while keeping the real APK filename. Verification: `npm run lint`, `npx tsc --noEmit`, and `npm run build` passed.
- Published CpIPOS installer assets through GitHub Releases after owner approval. Created `cpipos-windows-v0.3.3` with `CpIPOS.Desktop_0.3.3_x64-setup.exe` and `cpipos-android-v1.0.23-rc` as a prerelease with `CpIPOS-Android-POS-1.0.23-MDM-RC-DEBUG.apk`. Verification: public release downloads matched the local SHA-256 hashes (`ee3ca1ca13005eaac0d48f591e64eee63c93c540d03d53f3f20af8d8d2bff3dc` for Windows and `be39040de53bbd134ff958ef62faa932f1c48784906ef1f4d4216d1546e85d73` for Android), Android remained clearly marked RC/DEBUG, and no installer files were committed to source.
- Added production Vercel environment variables `CPIPOS_WINDOWS_DOWNLOAD_URL` and `CPIPOS_ANDROID_DOWNLOAD_URL`, redeployed production to `https://cuttingpointlnnovatlon-acgnvw3t2-sstdevelopaminnos-projects.vercel.app` (`dpl_5zBRhV7Nk3DLEcKC2hbSY3WYxuNS`), and reassigned `https://cuttingpointinnovation.vercel.app` to that latest deployment after Vercel auto-aliased only the hyphenated domain. Verification: `/`, `/downloads`, `/register-store`, and `/api/store-registration` returned HTTP 200; `/downloads` contained the GitHub release URLs and no longer showed the preparing-link fallback.
- Ran a production TEST store-registration submission through the company site API using package `Starter`. The request returned `201` with id `edeb0b89-0307-47a8-95a5-b30b7e51461c` and status `pending`; a duplicate POST using the same `submission_key` returned the same id with HTTP 200, confirming duplicate protection. Verification remained limited to public company-site/API behavior; IT Admin UI visibility and cleanup require authenticated CpIPOS-IT admin access.

## 2026-09-20
- Prepared separate CpIPOS trial signup navigation for company site: added a compact **สมัครใช้งาน** CTA after Contact (before Downloads) on desktop/mobile, TH/EN/LO copy, and `NEXT_PUBLIC_CPIPOS_SIGNUP_URL` pointing to the separate CpIPOS-IT `/register-store` application. The company website collects no applicant credentials or personal data; registration is handled by CpIPOS-IT and held for manual IT approval. Release gate: do not merge/deploy company-site signup navigation until CpIPOS-IT PR #40 is merged, `/register-store` is verified publicly and the IT review queue/activation flow is checked. No production site changed in this branch.

- Prepared production URL rename from `https://cuttingpointtech.vercel.app` to `https://cuttingpointinnovation.vercel.app` by updating default `NEXT_PUBLIC_SITE_URL` fallbacks, canonical metadata, sitemap/robots defaults, and project docs. Verification: old URL/name scans passed, `npm run lint` passed, and `npm run build` passed. Release result: committed `c211835`, pushed to GitHub `main`, deployed production to `https://cuttingpointlnnovatlon-f8g3xwcwb-sstdevelopaminnos-projects.vercel.app`, assigned `https://cuttingpointinnovation.vercel.app`, and also reassigned the legacy `https://cuttingpointtech.vercel.app` to the same latest build so old visitors do not see stale content. Verification: new URL returned HTTP 200 with `CPInno`, `ดาวน์โหลด`, `CUTTING POINT INNOVATION`, canonical metadata for `https://cuttingpointinnovation.vercel.app`, and no `CUTTING POINT TECH`; legacy URL returned HTTP 200 with the new brand and canonical metadata for the new URL; Vercel logs showed only 200 responses for the verification requests.
